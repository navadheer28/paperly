...
import { Upload, Download, Table, CheckCircle, AlertCircle } from 'lucide-react'
...
export default function PdfToExcel() {
  ...
  const handleConvert = async () => {
    ...
    try {
      const res = await pdfToExcel(file)
      setResult(res.data)
      setStatus('done')
    } catch (err) {
      setStatus('error')
    ...
    <div style={{ backgroundColor: '#FAFAF8' }} className="min-h-screen pt-28 pb-20 px-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-black text-gray-900 mb-3">PDF to Excel</h1>
          <p className="text-gray-500 text-lg">Extract tables from PDF into an Excel spreadsheet</p>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 mb-6 border border-gray-100">
          ...
          <div className="space-y-2">
            {[
              'Tables extracted into Excel sheets',
              'Each table on a separate sheet',
              'Ready to edit in Microsoft Excel',
              'Works with complex multi-page tables',
            ].map((item) => (
              <div key={item} className="flex items-center gap-3">
                <div style={{ backgroundColor: '#DCFCE7' }} className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0">
        ...
        {status === 'converting' && (
          <div className="bg-white rounded-2xl p-6 mb-6 border border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-700">Extracting tables from your PDF...</p>
                <p className="text-gray-400 text-sm mt-1">Large files may take a few minutes</p>
              </div>
            </div>
          </div>
        )}
